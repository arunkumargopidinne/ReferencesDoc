...
const PDFJS_STANDARD_FONT_DIR = pathToFileURL(`${path.join(
  PDF_PARSE_PDFJS_DIR,
  "standard_fonts"
)}${path.sep}`).href;

const PDFJS_BUNDLE_URL = pathToFileURL(path.join(
  PDF_PARSE_PDFJS_DIR,
  "legacy",
  "build",
  "pdf.mjs"
...
function getPdfParse() {
  ...
  const loaded = require("pdf-parse") as unknown;
  const parser =
    typeof loaded === "object" && loaded !== null && "PDFParse" in loaded
      ? loaded.PDFParse
      : null;

  if (typeof parser !== "function") throw new Error("Failed to initialize pdf parser");
  PdfParseClass = parser as PdfParseConstructor;
...
async function ocrPdf(buffer: Buffer): Promise<{ text: string; pages: number }> {
  ...
  const pdfjsLib = await import(PDFJS_BUNDLE_URL);
...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      console.error("Standard PDF parsing error:", getErrorMessage(error));
    }
    ...
    if (standardText.length < 50) {
      ...
      try {
        ...
        console.error("OCR Engine Error:", getErrorMessage(error));
        responseBody.warning = "No readable text found natively, and OCR fallback failed.";
      }
    }
    ...
    console.error("Endpoint failure:", getErrorMessage(error));
    return NextResponse.json({ error: getErrorMessage(error) || "Failed to extract PDF text" }, { status: 500 });
  }
}
